  ...
  try {
    ...
    */
    if (action === 'farm_full_db') {
      ...
      var incomingTime = time_(
        (incomingDb.meta && incomingDb.meta.updatedAt) ||
        data.updatedAt
      );

      var currentTime = currentDb
        ? time_(
            (currentDb.meta && currentDb.meta.updatedAt) ||
            ''
          )
        : 0;
      ...
      // آخر نسخة تكسب. لو النسخة القادمة أحدث أو مساوية، خزّنها وحدّث الشيتات.
      if (!currentDb || incomingTime >= currentTime) {
        writeCloudDb_(incomingDb);

        updateSheetsFromFullDb_(incomingDb);
