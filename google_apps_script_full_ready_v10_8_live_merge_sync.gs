...
var FARM_PAYMENT_LOG_SHEET = 'سجل سداد العملاء';
  ...
  try {
    ...
    */
    if (action === 'farm_full_db') {
      ...
      // v10.7: دمج وليس استبدال. كده لو الموبايل أضاف مصروف والكمبيوتر رفع نسخة قديمة، المصروف لا يضيع.
      var dbToSave = currentDb
...
function mergeArrayServer_(currentArr, incomingArr) {
  var map = {};
  var order = [];

  function put(record, preferIncomingOnNewer) {
    if (!record || !record.id) {
      return;
    }
    ...
    // لو السجل القادم أحدث، ناخده. لو نفس التوقيت نسيب الموجود في السحابة لحماية البيانات من جهاز قديم.
  ...
  (currentArr || []).forEach(function (record) {
    put(record, false);
  });

  (incomingArr || []).forEach(function (record) {
    put(record, true);
  });
